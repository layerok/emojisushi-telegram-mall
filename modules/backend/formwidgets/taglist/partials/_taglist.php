...
<?php else: ?>
    ...
    <select
        ...
        <?php if (!empty($placeholder)): ?>data-placeholder="<?= e(__($placeholder)) ?>"<?php endif ?>
        multiple
        <?= $field->getAttributes() ?>>
        <?php foreach ($availableOptions as $key => $option): ?>
            <?php if (!strlen($option)) continue ?>
            <?php if ($useKey): ?>
                <option value="<?= e($key) ?>" <?= in_array($key, $selectedValues) ? 'selected="selected"' : '' ?>><?= e(__($option)) ?></option>
            <?php else: ?>
                <option value="<?= e($option) ?>" <?= in_array($option, $selectedValues) ? 'selected="selected"' : '' ?>><?= e(__($option)) ?></option>
            <?php endif ?>
        <?php endforeach ?>
    </select>
<?php endif ?>
